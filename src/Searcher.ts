...
export class Searcher
{
    public search<T>(query : string, searcherTypeClass : BaseSearcher<T>) : T[]
    {
		...
		result = searcherTypeClass.search(StringRewriter.changeRussianToEngilshTranslirization(query));

		if(result.length > 0)
			return result;

		result = result = searcherTypeClass.search(StringRewriter.changeEngilshToRussianTranslirization(query));
